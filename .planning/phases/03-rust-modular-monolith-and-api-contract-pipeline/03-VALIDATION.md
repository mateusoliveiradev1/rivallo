...
| 03-05-01 | 05 | 4 | DATA-02 | T-03-06 | OpenAPI export is deterministic; drift check is isolated, byte-based, actionable, and non-mutating. | Node integration | `pnpm contracts:openapi:generate && pnpm contracts:openapi:check && pnpm test -- tooling-tests/openapi-pipeline.test.mjs` | ❌ created by task | ⬜ pending |
| 03-06-01 | 06 | 5 | DATA-02 | T-03-07 | Generated-only contracts client derives types, metadata, and a minimal client exclusively from committed OpenAPI. | Generation/typecheck | `pnpm contracts:client:generate && pnpm typecheck` | ❌ created by task | ⬜ pending |
| 03-06-02 | 06 | 5 | DATA-02 | T-03-08 | Client drift check uses isolated output and rejects hand-authored schema duplicates. | Node integration | `pnpm contracts:client:generate && pnpm contracts:client:check && pnpm test -- tooling-tests/contracts-client-generation.test.mjs` | ❌ created by task | ⬜ pending |
