...
**Status:** canonical planning contract amended on 2026-07-16 for Phase 06.1 engine/current-local integration and Phase 9 career-data/cache/offline integration. This document does not implement the engine.
...
6. Phase 5 owns this contract and the bounded DenseTable primitive only. Phase 06.1 owns the real controlled engine, Elenco screen contract and a versioned adapter/repository for the current fixed local owner. Phase 9 owns migration to career identity/SQLite plus real data, query-scale, cache, offline and synchronisation integration.
...
Phase 06.1 owns the first adapter/repository, versioned persistence location, sequential schema migrations, quarantine policy and evidence under an explicit `local-fixed` owner scope. That scope is a migration seam, not a multi-user claim. Phase 9 replaces or migrates the storage implementation when career identity and SQLite/cache/offline ownership arrive. React and the Table View component never access SQLite, `localStorage`, files, or network storage directly.

## Keyboard, Focus, and Announcements

Every pointer operation has a documented keyboard equivalent in the Phase 06.1 owning-screen contract:
...
Virtualization and pagination must preserve semantic table relationships, keyboard reachability, focused-row recovery, loading/empty/error/offline states, finite width, and truthful row/position metadata. Phase 06.1 may prove only the bounded client mode justified by the current fixed-club fixture. Phase 9 selects and proves the production mode from real career data scale and owns query/cache/offline integration; Phase 5 creates no virtualization or query implementation.
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `tableId`                  | The stable squad/player table ID declared by its Phase 06.1 screen contract.                                                                 |
...
The owning Phase 06.1 screen contract decides whether the phrase means “show only identity and goals columns” or also “filter to players with goals.” That decision is represented by ordinary columns/filter/sort clauses. There is no `goalsOnly` flag, special component, route condition, hardcoded renderer, or one-off query branch.

## Cross-Phase Responsibility and Acceptance Matrix

| Contract area                    | Phase 5 — contract owner                                                                                                        | Phase 06.1 — current product owner                                                                                                                              | Phase 9 — career integration/hardening owner                                                                                                       | Required verification evidence                                                                  |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Visual/primitive boundary        | Link to `DESIGN.md`/`05-UI-SPEC.md`; keep DenseTable finite and semantic.                                                       | Apply canonical rules to Elenco and the view customizer in every state and target viewport.                                                                      | Preserve rules with real career data and cache/offline states.                                                                                     | Phase 5 structural evidence; 06.1 visual/a11y evidence; Phase 9 offline/scale evidence.          |
| Table/schema identity            | Define stable IDs, state grammar, validation, and bounds.                                                                       | Assign `tableId`, schema version, stable columns, required identity/actions and capabilities; implement controlled state without semantic drift.                  | Bind identity to career/user scope and real data projections without changing table semantics.                                                     | Schema, migration and drift tests in 06.1 and Phase 9.                                          |
| Columns, filters, sort, grouping | Define generic controlled operations.                                                                                           | Implement supported Elenco operations and default/system views through commands and the bounded current-client data mode.                                        | Select and prove production data/query/window modes from measured career scale.                                                                    | 06.1 behavior tests; Phase 9 query/scale evidence.                                              |
| Saved views and provenance       | Define lifecycle, provenance, normalization, dirty/reset semantics.                                                             | Implement the repository, `local-fixed` versioned persistence, sequential migrations, quarantine and recovery behind a port.                                     | Migrate owner scope/storage to career identity and SQLite/synchronisation boundaries.                                                              | 06.1 restart/corruption/migration evidence; Phase 9 identity/storage migration evidence.        |
| Accessibility and states         | Define keyboard equivalence, bounded tab stops, focus retention, live announcements, and non-colour cues.                       | Implement pointer/keyboard parity plus truthful loading/empty/error/recovery states for the current fixed-club surface.                                           | Integrate real offline, stale, queued, synchronising and reconnected states.                                                                        | Keyboard, focus, contrast, reduced-motion and state evidence in both phases.                      |
| Data windows and queries         | Define four explicit modes, capability negotiation, stable rows/selection, serialization, cancellation, and stale-result rules. | Declare and prove only the bounded client mode justified by the current fixed fixture; retain stable rows/selection and deterministic descriptors.               | Choose the proven production mode and implement query, pagination/virtualization, cache/offline, cancellation and selection integration.           | 06.1 bounded-mode evidence; Phase 9 adapter/repository/query and scale evidence.                 |
| `Mostrar somente gols`           | Define it as a general configured saved-view example.                                                                           | Resolve exact columns/filter/tie-breaker and execute it through the ordinary local engine pipeline with no special branch.                                         | Preserve the same semantics through the real career query pipeline.                                                                                | 06.1 saved-view integration test; Phase 9 general-query integration test.                        |
...
- **Phase 06.1 owner:** [Roadmap Phase 06.1](../../ROADMAP.md#phase-061-sm-2-table-view-engine-and-durable-views-inserted) must produce the real controlled engine, Elenco screen contract, `local-fixed` repository, versioned persistence/migrations, interaction/state/viewport fixtures and corruption/recovery evidence.
- **Phase 9 owner:** [Roadmap Phase 9](../../ROADMAP.md#phase-9-dashboard-squad-cache-and-offline-hardening) must migrate owner/storage to career identity and SQLite as applicable, integrate real data/query/window/cache/offline/synchronisation behavior, and preserve contract semantics with scale/drift evidence.
