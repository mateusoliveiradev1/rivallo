...
impl Default for MatchdayState {
    fn default() -> Self {
        crate::world::bundled_official_world().matchday
    }
}
...
fn legacy_hardcoded_matchday_fixture() -> MatchdayState {
    let starters = HashSet::from([
        "rv-01", "rv-02", "rv-03", "rv-04", "rv-05", "rv-06", "rv-07", "rv-08", "rv-09", "rv-10",
        "rv-11",
    ]);
    let player = |id: &str,
                  name: &str,
    ...
    pub fn play_next_match(&mut self) -> Result<MatchResult, MatchdayError> {
        ...
        for goal in 0..home_goals {
            ...
            events.push(MatchEvent {
                ...
                kind: "goal".to_owned(),
                text: format!(
                    "Gol do {} — {} concluiu a jogada.",
                    self.club.short_name, scorer.short_name
                ),
                for_user_club: true,
            });
        }
        for goal in 0..away_goals {
            events.push(MatchEvent {
                minute: 31 + goal * 23,
                kind: "goal".to_owned(),
                text: format!("Gol do {} em transição rápida.", self.opponent.short_name),
                for_user_club: false,
            });
        }
